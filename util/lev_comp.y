...
function_call	:
NQSTRING '(' func_call_params_list ')' {
...
forstmt_start	:
FOR_ID any_var_or_unk '=' math_expr_var for_to_span math_expr_var {
...
wallwalk_detail	:
WALLWALK_ID ':' coord_or_var ',' mapchar_or_var opt_spercent {
...
corridor	:
CORRIDOR_ID ':' corr_spec ',' corr_spec {
...
room_begin      :
room_type opt_percent ',' light_state {
    if (($2 < 100) && ($1 == OROOM))
...
subroom_def	:
SUBROOM_ID ':' room_begin ',' subroom_pos ',' room_size optroomregionflags {
    long flags = $8;
...
room_def	:
ROOM_ID ':' room_begin ',' room_pos ',' room_align ',' room_size optroomregionflags {
    long flags = $8;
    if (flags == -1) flags = (1 << 0);
...
door_detail	:
ROOMDOOR_ID ':' secret ',' door_state ',' door_wall ',' door_pos {
...
sounds_detail	:
SOUNDS_ID ':' integer_or_var ',' sounds_list {
    long n_sounds = $5;
...
lvl_sound_part	: '('
MSG_OUTPUT_TYPE ',' string_expr ')' {
    add_opvars(splev, "i", (long)$2);
}
;

mon_generation	:
MON_GENERATION_ID ':' SPERCENT ',' mon_gen_list {
    long total_mons = $5;
...
monster_desc	:
monster_or_var ',' coord_or_var monster_infos {
    /* nothing */
}
;
...
trap_detail	:
TRAP_ID ':' trap_name ',' coord_or_var {
    add_opvars(splev, "io", (long)$3, SPO_TRAP);
}
;

drawbridge_detail:
DRAWBRIDGE_ID ':' coord_or_var ',' DIRECTION ',' door_state {
    long d, state = 0;
...
mazewalk_detail :
MAZEWALK_ID ':' coord_or_var ',' DIRECTION {
...
ladder_detail	:
LADDER_ID ':' coord_or_var ',' UP_OR_DOWN {
    add_opvars(splev, "io", (long)$<i>5, SPO_LADDER);
}
;

stair_detail	:
STAIR_ID ':' coord_or_var ',' UP_OR_DOWN {
    add_opvars(splev, "io", (long)$<i>5, SPO_STAIR);
}
;

stair_region	:
STAIR_ID ':' lev_region ',' lev_region ',' UP_OR_DOWN {
...
portal_region	:
PORTAL_ID ':' lev_region ',' lev_region ',' STRING {
    add_opvars(splev, "iiiii iiiii iiso",
    $3.x1, $3.y1, $3.x2, $3.y2, $3.area,
    $5.x1, $5.y1, $5.x2, $5.y2, $5.area,
...
teleprt_region	:
TELEPRT_ID ':' lev_region ',' lev_region teleprt_detail {
    long rtype = 0;
    switch($6) {
    case -1:
        rtype = LR_TELE;
...
branch_region	:
BRANCH_ID ':' lev_region ',' lev_region {
    add_opvars(splev, "iiiii iiiii iiso",
    $3.x1, $3.y1, $3.x2, $3.y2, $3.area,
    $5.x1, $5.y1, $5.x2, $5.y2, $5.area,
...
fountain_detail :
FOUNTAIN_ID ':' ter_selection {
    add_opvars(splev, "o", SPO_FOUNTAIN);
}
;

sink_detail :
SINK_ID ':' ter_selection {
    add_opvars(splev, "o", SPO_SINK);
}
;

pool_detail :
POOL_ID ':' ter_selection {
...
replace_terrain_detail :
REPLACE_TERRAIN_ID ':' region_or_var ',' mapchar_or_var ',' mapchar_or_var ',' SPERCENT {
    add_opvars(splev, "io", $9, SPO_REPLACETERRAIN);
}
;

terrain_detail :
TERRAIN_ID ':' ter_selection ',' mapchar_or_var {
    add_opvars(splev, "o", SPO_TERRAIN);
}
;

spill_detail :
SPILL_ID ':' coord_or_var ',' mapchar_or_var ',' DIRECTION ',' math_expr_var {
    add_opvars(splev, "io", (long)$7, SPO_SPILL);
}
;

diggable_detail :
NON_DIGGABLE_ID ':' region_or_var {
    add_opvars(splev, "o", SPO_NON_DIGGABLE);
}
;

passwall_detail :
NON_PASSWALL_ID ':' region_or_var {
    add_opvars(splev, "o", SPO_NON_PASSWALL);
}
;

region_detail	:
REGION_ID ':' region_or_var ',' light_state ',' room_type optroomregionflags {
    long irr;
    long rt = $7;
    long flags = $8;
    if (flags == -1) flags = (1 << 0);
...
altar_detail	:
ALTAR_ID ':' coord_or_var ',' alignment ',' altar_type {
    add_opvars(splev, "iio", (long)$7, (long)$5, SPO_ALTAR);
}
;

grave_detail	:
GRAVE_ID ':' coord_or_var ',' string_expr {
...
gold_detail	:
GOLD_ID ':' math_expr_var ',' coord_or_var {
    add_opvars(splev, "o", SPO_GOLD);
}
;

engraving_detail:
ENGRAVING_ID ':' coord_or_var ',' engraving_type ',' string_expr {
    add_opvars(splev, "io",
...
mineralize	:
MINERALIZE_ID ':' integer_or_var ',' integer_or_var ',' integer_or_var ',' integer_or_var {
...
func_param_part		:
any_var_or_arr ':' func_param_type {
