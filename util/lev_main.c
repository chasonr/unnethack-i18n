...
struct opvar *
set_opvar_int(struct opvar *ov, long int val)
{
    if (ov) {
...
struct opvar *
set_opvar_coord(struct opvar *ov, long int val)
{
    if (ov) {
...
struct opvar *
set_opvar_region(struct opvar *ov, long int val)
{
    if (ov) {
...
struct opvar *
set_opvar_mapchar(struct opvar *ov, long int val)
{
    if (ov) {
...
struct opvar *
set_opvar_monst(struct opvar *ov, long int val)
{
    if (ov) {
...
struct opvar *
set_opvar_obj(struct opvar *ov, long int val)
{
    if (ov) {
...
void
break_stmt_new(sp_lev *splev, long int i)
{
...
struct lc_funcdefs *
funcdef_new(long int addr, char *name)
{
...
struct lc_vardefs *
vardef_new(long int typ, char *name)
{
...
void
check_vardef_type(struct lc_vardefs *vd, char *varname, long int vartype)
{
    struct lc_vardefs *tmp;
...
struct lc_vardefs *
add_vardef_type(struct lc_vardefs *vd, char *varname, long int vartype)
{
    struct lc_vardefs *tmp;
    if ((tmp = vardef_defined(vd, varname, 1))) {
        if (tmp->var_type != vartype)
...
what_map_char(char c)
{
    ...
    case 'M'  :
        return(BOG);	/* muddy swamp */
    case 'x'  :
        return(MAX_TYPE);	/* 'see-through' */
    case 'Y'  :
        return(CRYSTALICEWALL);
    case 'U'  :
        return(ICEWALL);
    }
    return(INVALID_TYPE);
 ...
 */
static boolean
write_common_data(int fd, sp_lev *lvl)
{
        ...
        if (tmpo.opcode == SPO_PUSH) {
            ...
            if (opdat) {
                ...
                case SPOVAR_STRING:
                    ...
                    break;
                default:
                    panic("decompile_maze: unknown data type (%i).", ov->spovartyp);
                }
