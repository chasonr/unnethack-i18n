...
struct xwindow *wp;
{
    ...
    for (i = 0, sv = shown_stats; i < NUM_STATS; i++, sv++) {
        ...
        case F_CONFUSED:
            ...
            break;
        case F_SICK:
            val = (long) Sick ? (long)u.usick_type
                  : 0L;
            break;
        case F_BLIND:
...
#endif
        case F_HP:
            val = (long) (u.mtimedone ?
                          (u.mh  > 0 ? u.mh  : 0):
                              (u.uhp > 0 ? u.uhp : 0));
            break;
        case F_MAXHP:
            val = (long) (u.mtimedone ? u.mhmax :
                          u.uhpmax);
            break;
        case F_POWER:
            val = (long) u.uen;
...
int *col_indices;
{
    ...
    num_args = 0;
    if (top != (Widget) 0) {
        XtSetArg(args[num_args], XtNfromVert, top);
        num_args++;
    }
    if (left != (Widget) 0) {
        XtSetArg(args[num_args], XtNfromHoriz, left);
        num_args++;
    }
    XtSetArg(args[num_args], XtNdefaultDistance, 0);
    num_args++;
...
Widget parent, top, left;
{
    ...
    num_args = 0;
    XtSetArg(args[num_args], XtNfromVert, sv_name->w);
    num_args++;
