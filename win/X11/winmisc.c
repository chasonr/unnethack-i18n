...
{
    ps_selected = (int)(intptr_t) client_data;
    exit_x_event = TRUE;		/* leave event loop */
}
...
{
    int selected = (int)(intptr_t) client_data;
        ...
        Widget        *formp)           /* return */
{
    ...
    *formp = form = XtCreateManagedWidget("menuform",
    ...
    num_args = 0;
    XtSetArg(args[num_args], XtNdefaultDistance, &distance);
    num_args++;
    ...
    */
    ...
    for (i = 0, above = left, curr = commands; i < num_names; i++) {
        ...
        num_args = 0;
        XtSetArg(args[num_args], XtNfromVert, above);
        num_args++;
        if (above == left) {
            /* if first, we are farther apart */
            XtSetArg(args[num_args], XtNvertDistance, skip);
            num_args++;
        }
        ...
        *curr = XtCreateManagedWidget(widget_names[i],
                                      ...
                                      form,
                                      args, num_args);
        XtAddCallback(*curr, XtNcallback, name_callback, (XtPointer)(intptr_t) i);
        above = *curr++;
    }
