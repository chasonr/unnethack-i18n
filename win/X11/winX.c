...
{
    if (x_inited) XFlush(XtDisplay(toplevel));
}
...
{
    return;    /* called from options.c */
}


void X11_start_screen(void)
{
    return;    /* called from setftty() in unixtty.c */
}
void X11_end_screen(void)
{
    return;    /* called from settty() in unixtty.c */
}
...
void
#ifdef FILE_AREAS
X11_display_file(const char *farea, const char *str, boolean complain)
#else
...
#define LLEN 128
    ...
    num_args = 0;
    XtSetArg(args[num_args], XtNtitle, str);
    num_args++;
    ...
    XtSetArg(args[num_args], XtNscrollVertical,
             XawtextScrollWhenNeeded);
    num_args++;
    XtSetArg(args[num_args], XtNtype, XawAsciiString);
    num_args++;
    XtSetArg(args[num_args], XtNstring, textlines);
    num_args++;
    XtSetArg(args[num_args], XtNdisplayCaret, False);
    num_args++;
    ...
    num_args++;
    XtSetArg(args[num_args], XtNtopMargin,    &top_margin);
    num_args++;
...
char
X11_yn_function(const char *ques, const char *choices, char def)
{
    static Boolean need_to_init = True;
    char buf[QBUFSZ];
    Arg args[4];
...
/*ARGSUSED*/
static void
win_visible(Widget w, XtPointer data, XEvent *event, Boolean *flag)	/* only called for autofocus */
...
init_standard_windows(void)
{
    ...
    if(appResources.slow) {
        ...
        yn_label = XtCreateManagedWidget("yn_label",
                                         ...
                                         args, num_args);
        num_args = 0;
        XtSetArg(args[num_args], XtNfromVert, message_viewport);
        num_args++;
     ...
     */
    num_args = 0;
    XtSetArg(args[num_args], XtNfromVert, map_viewport);
    num_args++;
    XtSetArg(args[num_args], XtNleft,	  XtChainLeft);
    num_args++;
    XtSetArg(args[num_args], XtNright,	  XtChainLeft);
    num_args++;
    XtSetArg(args[num_args], XtNtop,	  XtChainBottom);
    num_args++;
    XtSetArg(args[num_args], XtNbottom,	  XtChainBottom);
    num_args++;
...
nh_XtPopup(Widget w, int g, Widget childwid)
         		/* widget */
         		/* type of grab */
                	/* child to receive focus (can be None) */
{
