...
mapseen *
find_level_by_custom_name(const char *s)
{
    mapseen *mptr;
 ...
 */
s_level *
get_next_elemental_plane(d_level *lev)
{
    if (!In_endgame(lev)) {
 ...
 */
d_level *
get_first_elemental_plane(void)
{
    s_level *curr,
...
#ifdef RANDOMIZED_PLANES
void
shuffle_planes(void) /* randomizes order of elemental planes */
{
    /* original order */
...
xchar
dunlev(d_level *lev)	/* return the level number for lev in *this* dungeon */
...
xchar
dunlevs_in_dungeon(d_level *lev)	/* return the lowest level number for *this* dungeon*/
...
xchar
deepest_lev_reached(boolean noquest) /* return the lowest level explored in the game*/
...
boolean
on_level(d_level *lev1, d_level *lev2)	/* are "lev1" and "lev2" actually the same? */
...
boolean
In_quest(d_level *lev)	/* are you in the quest dungeon? */
...
boolean
In_mines(d_level *lev)	/* are you in the mines dungeon? */
...
boolean
In_sheol(d_level *lev)	/* are you in Sheol? */
...
boolean
In_V_tower(d_level *lev)	/* is `lev' part of Vlad's tower? */
...
boolean
On_W_tower_level(d_level *lev)	/* is `lev' a level containing the Wizard's tower? */
...
boolean
In_W_tower(int x, int y, d_level *lev)	/* is <x,y> of `lev' inside the Wizard's tower? */
...
boolean
In_hell(d_level *lev)	/* are you in one of the Hell levels? */
...
void
find_hell(d_level *lev)	/* sets *lev to be the gateway to Gehennom... */
...
void
goto_hell(boolean at_stairs, boolean falling)	/* go directly to hell... */
...
void
assign_level(d_level *dest, d_level *src)		/* equivalent to dest = source */
...
void
assign_rnd_level(d_level *dest, d_level *src, int range)	/* dest = src + rn1(range) */
...
br_string2(branch *br)
{
    ...
    case BR_NO_END1:
        return "Connection";
    case BR_NO_END2:
        return (br->end1_up) ? "One way stairs up" :
               "One way stairs down";
    case BR_STAIR:
...
STATIC_OVL void
print_mapseen(winid win, mapseen *mptr, boolean printdun, boolean dump, boolean final)
          
              
                 
              /**< if information should be dumped to file */
               /**< if game is finished */
{
    char buf[BUFSZ];
    int i, depthstart;
...
const char *
get_generic_level_description(d_level *lev)
{
