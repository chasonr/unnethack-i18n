...
mon_tele:
    ...
    default:
        warning("%s wanted to perform action %d?", Monnam(mtmp),
                m.has_defense);
        break;
    }
    return 0;
 ...
 */
STATIC_OVL void
mbhit(struct monst *mon, register int range, int (*fhitm) (struct monst *, struct obj *), int (*fhito) (struct obj *, struct obj *), struct obj *obj)
