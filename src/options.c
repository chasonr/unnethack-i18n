...
parse_status_color_option(char *start)
{
    ...
    *middle++ = '\0';
    if (middle - start > 2 && (start[2] == '%' || start[2] == '.' || start[2] == '<' || start[2] == '>')) {
        ...
        default:
        case '%':
            percent_color_option->statclrtype = STATCLR_TYPE_PERCENT;
            break;
        case '.':
            percent_color_option->statclrtype = STATCLR_TYPE_NUMBER_EQ;
            break;
        case '>':
            percent_color_option->statclrtype = STATCLR_TYPE_NUMBER_GT;
            break;
        case '<':
            percent_color_option->statclrtype = STATCLR_TYPE_NUMBER_LT;
            break;
        }
...
STATIC_OVL void
duplicate_opt_detection(const char *opts, int bool_or_comp)
...
static boolean
parse_extended_option(const char *str, char *option_name, char *option_value)
                
                  	/**< Output string buffer for option name */
                   	/**< Output string buffer for option value */
{
    int i;
    char *tmps, *cs;
    char buf[BUFSZ];
