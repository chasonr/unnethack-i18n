    ...
    if (num > 0 && num < TRAPNUM) {
        ...
        do {
            ...
            case POLY_TRAP:
                ...
                break;
            case FIRE_TRAP:
                if (!Inhell || Insheol) kind = NO_TRAP;
                break;
            case ICE_TRAP:
                if (!Insheol) kind = NO_TRAP;
                break;
            case TELEP_TRAP:
 ...
 */
void
place_random_engraving(const char *const *engravings, const const int size)
                                
                /* number of engravings in array */
{
...
place_random_engravings(void)
{
    switch (depth(&u.uz)) {
    case 23:
        place_random_engraving(illuminati_engravings, SIZE(illuminati_engravings));
        break;
    case 42:
        place_random_engraving(hhgtg_engravings, SIZE(hhgtg_engravings));
        break;
    }
}
