...
spill_terrain(spill *sp, struct mkroom *croom)
{
    ...
    if (sp->x < 0 || sp->y < 0) {
        for (j = 0; j < 500; j++) {
            ...
            case W_NORTH:
                ny++;
                break;	  /* backwards to make sure we're against a wall */
            case W_SOUTH:
                ny--;
                break;
            case W_WEST:
...
spo_corefunc(struct sp_coder *coder, long int fn)
{
    ...
    switch (fn) {
    default:
        impossible("Unknown sp_lev core function %li", fn);
        break;
    case COREFUNC_LEVEL_DIFFICULTY:
        i = opvar_new_int(level_difficulty());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_LEVEL_DEPTH:
        i = opvar_new_int(depth(&u.uz));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_DISCORDIAN_HOLIDAY:
        i = opvar_new_int(discordian_holiday());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_PIRATEDAY:
        i = opvar_new_int(pirateday());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_APRILFOOLSDAY:
        i = opvar_new_int(aprilfoolsday());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_PIDAY:
        i = opvar_new_int(piday());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_TOWELDAY:
        i = opvar_new_int(towelday());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_MIDNIGHT:
        i = opvar_new_int(midnight());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_NIGHT:
        i = opvar_new_int(night());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_FRIDAY_13TH:
        i = opvar_new_int(friday_13th());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_POM:
        i = opvar_new_int(phase_of_the_moon());
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_YYYYMMDD:
        i = opvar_new_int(yyyymmdd(0));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_PLNAME:
        s = opvar_new_str(plname);
        splev_stack_push(coder->stack, s);
        break;
    case COREFUNC_ROLE:
        s = opvar_new_str(urole.name.m);
        splev_stack_push(coder->stack, s);
        break;
    case COREFUNC_RACE:
        s = opvar_new_str(urace.noun);
        splev_stack_push(coder->stack, s);
        break;
    case COREFUNC_ROOM_WID:
        i = opvar_new_int((coder->croom ? (coder->croom->hx - coder->croom->lx + 1) : 0));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_ROOM_HEI:
        i = opvar_new_int((coder->croom ? (coder->croom->hy - coder->croom->ly + 1) : 0));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_ROOM_X:
        i = opvar_new_int((coder->croom ? coder->croom->lx : 0));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_ROOM_Y:
        i = opvar_new_int((coder->croom ? coder->croom->ly : 0));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_ARRAY_LEN:
        i = opvar_new_int(opvar_array_length(coder));
        splev_stack_push(coder->stack, i);
        break;
    case COREFUNC_RN2:
        if (OV_pop_i(i)) {
...
void
spo_end_moninvent(struct sp_coder *coder)
{
    if (invent_carrying_monster)
...
void
spo_pop_container(struct sp_coder *coder)
{
    if (container_idx > 0) {
        container_idx--;
        container_obj[container_idx] = NULL;
    ...
    for (x = 0; x < COLNO; x++)
        for (y = 0; y < ROWNO; y++)
            if (selection_getpoint(x,y,ov) && (levl[x][y].typ == mapc)) {
                ...
                case 0:
                case 1:
                    if (levl[x][y].lit == lit) selection_setpoint(x,y,ret, 1);
                    break;
                }
            }
    return ret;
...
int
selection_rndcoord(struct opvar *ov, schar *x, schar *y)
{
    int idx = 0;
    int c;
    int dx,dy;
...
void
selection_floodfill(struct opvar *ov, int x, int y)
{
...
void
selection_do_line(schar x1, schar y1, schar x2, schar y2, struct opvar *ov) /* bresenham line algo */
...
void
sel_set_wallify(int x, int y, genericptr_t arg)
{
    wallify_map(x,y, x,y);
}
