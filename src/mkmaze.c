 ...
 */
STATIC_OVL int
extend_spine(int (*locale)[3], int wall_there, int dx, int dy)
{
    int spine, nx, ny;
...
STATIC_OVL void
maze0xy(coord *cc)	/* find random starting point for maze generation */
        ...
        if (dunlev(&u.uz)==1) {
            ...
            for (ep = head_engr; ep; ep = ep->nxt_engr) {
                ...
                if (ep && ep->engr_txt[0] && ep->engr_type == BURN &&
                    ...
                    switch (price) {
                    case 0:
                        price_obj = BAG_OF_HOLDING;
                        break;
                    case 1:
                        price_obj = rn2(2) ? CLOAK_OF_MAGIC_RESISTANCE : CLOAK_OF_DISPLACEMENT;
                        break;
                    case 2:
                        price_obj = !rn2(3) ? AMULET_OF_LIFE_SAVING : rn2(2) ? AMULET_OF_FLYING : AMULET_OF_REFLECTION;
                        break;
                    }
...
#ifdef ADVENT_CALENDAR
void
fill_advent_calendar(boolean init)
{
    int door_nr=1;
    char buf[4];
    int x,y;
                ...
                if (init) {
                    ...
                    if (door_nr == 24) {
                        ...
                        case  9:
                            ...
                            break;
                        case 10:
                            object = BAG_OF_TRICKS;
                            break;
                        case 11:
                            object = EXPENSIVE_CAMERA;
                            break;
                        case 12:
                            object = HORN_OF_PLENTY;
                            break;
                        case 13:
                            object = STETHOSCOPE;
                            break;
...
void
mazexy(coord *cc)	/* find random point in generated corridors,
 ...
 */
{
    ...
    found = nonwall = FALSE;
    for(ymin=0; !found && (ymin < COLNO); ymin++) {
        lev = &levl[xmin][ymin];
...
void
mkportal(register xchar x, register xchar y, register xchar todnum, register xchar todlevel)
{
