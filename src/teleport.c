 ...
 */
static int
badpos(int x, int y, struct monst *mtmp, unsigned int gpflags)
{
 ...
 */
boolean
goodpos(int x, int y, struct monst *mtmp, unsigned int gpflags)
{
    struct permonst *mdat = NULL;
    boolean ignorewater = ((gpflags & MM_IGNOREWATER) != 0);
...
boolean
enexto_core(coord *cc, register xchar xx, register xchar yy, struct permonst *mdat, unsigned int entflags)
{
    return enexto_core_range(cc, xx, yy, mdat, entflags, 1);
}

boolean
enexto_core_range(coord *cc, register xchar xx, register xchar yy, struct permonst *mdat, unsigned int entflags, int start_range)
          
                      
                      
                  
                 /**< Distance of checked tiles to begin with. Should be >=1. */
{
#define MAX_GOOD 15
