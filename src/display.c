 ...
 */
STATIC_OVL void
display_monster(register xchar x, register xchar y, register struct monst *mon, int sightflags, register xchar worm_tail)
                    	/* display position */
                           	/* monster to display */
...
STATIC_PTR uchar get_glyph_char(int glyph, int *oclass)
{
    uchar   ch;
    register int offset;
    *oclass = 0;
    ...
    for (y = 0; y < ROWNO; y++) {
        ...
        for (x = 1; x < COLNO; x++) {
            ...
            if (strlen(tmpbuf) > BUFSIZE_PER_SQUARE) {
                warning("tmpbuf > %d: %lu, %s", BUFSIZE_PER_SQUARE, (unsigned long)strlen(tmpbuf), tmpbuf);
            }
            /* HTML map */
